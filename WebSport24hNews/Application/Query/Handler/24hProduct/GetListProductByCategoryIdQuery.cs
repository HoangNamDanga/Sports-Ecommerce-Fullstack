...
{
    public class GetListProductByCategoryIdQuery : IQueryBase<IEnumerable<DhnProductDto>>
    {
        public decimal? categoryId { get; set; }
    }
    public class GetListProductByCategoryIdQueryHandler : IRequestBaseHandler<GetListProductByCategoryIdQuery, IEnumerable<DhnProductDto>>
    {
        ...
        public GetListProductByCategoryIdQueryHandler(IRepositoryService repositoryService, IMapper mapper, IAuthorizeExtensionService authorizeExtension)
        {
        ...
        public async Task<IEnumerable<DhnProductDto>> Handle(GetListProductByCategoryIdQuery request, CancellationToken cancellationToken)
        {
            if (request is null)
            ...
            var productsWithThumbnails = await(from p in _repositoryService.Table<DhnProduct>()
                                               where p.CategoryId == request.categoryId // <-- ĐÂY LÀ ĐIỂM KHÁC BIỆT
                                               select new
                                               {
                                                   Product = p,
            ...
            var groupedResult = productsWithThumbnails
                .Select(pWithThumb => new DhnProductDto
                {
                    ...
                    CategoryId = pWithThumb.Product.CategoryId,
            ...
            groupedResult = groupedResult.Take(5).ToList();
