...
{
    public class GetListProductShoesSoccerHomeQuery : IQueryBase<IEnumerable<DhnProductDto>>
    {
    }
    public class GetListProductShoesSoccerHomeQueryHandler : IRequestBaseHandler<GetListProductShoesSoccerHomeQuery, IEnumerable<DhnProductDto>>
    {
        ...
        public GetListProductShoesSoccerHomeQueryHandler(IRepositoryService repositoryService, IMapper mapper, IAuthorizeExtensionService authorizeExtension)
        {
        ...
        public async Task<IEnumerable<DhnProductDto>> Handle(GetListProductShoesSoccerHomeQuery request, CancellationToken cancellationToken)
        {
            if (request is null)
            ...
            var productsWithThumbnails = await(from p in _repositoryService.Table<DhnProduct>()
                                               where p.CategoryId == 4 // <-- ĐÂY LÀ ĐIỂM KHÁC BIỆT
                                               select new
                                               {
                                                   Product = p,
