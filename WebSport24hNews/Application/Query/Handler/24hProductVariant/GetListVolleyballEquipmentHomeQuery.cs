...
{
    public class GetListVolleyballEquipmentHomeQuery : IQueryBase<IEnumerable<DhnProductDto>>
    {
    }
    public class GetListVolleyballEquipmentHomeQueryHandler : IRequestBaseHandler<GetListVolleyballEquipmentHomeQuery, IEnumerable<DhnProductDto>>
    {
        ...
        public GetListVolleyballEquipmentHomeQueryHandler(IRepositoryService repositoryService, IMapper mapper, IAuthorizeExtensionService authorizeExtension)
        {
        ...
        public async Task<IEnumerable<DhnProductDto>> Handle(GetListVolleyballEquipmentHomeQuery request, CancellationToken cancellationToken)
        {
            if (request is null)
            ...
            var productsWithThumbnails = await(from p in _repositoryService.Table<DhnProduct>()
                                               where p.CategoryId == 9 // <-- ĐÂY LÀ ĐIỂM KHÁC BIỆT
                                               select new
                                               {
                                                   Product = p,
