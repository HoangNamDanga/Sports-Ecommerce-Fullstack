...
{
    public class GetListProductTrainingHomeQuery : IQueryBase<IEnumerable<DhnProductDto>>
    {
    }

    public class GetListProductTrainingHomeQueryHandler : IRequestBaseHandler<GetListProductTrainingHomeQuery, IEnumerable<DhnProductDto>>
    {
        ...
        public GetListProductTrainingHomeQueryHandler(IRepositoryService repositoryService, IMapper mapper, IAuthorizeExtensionService authorizeExtension)
        {
        ...
        public async Task<IEnumerable<DhnProductDto>> Handle(GetListProductTrainingHomeQuery request, CancellationToken cancellationToken)
        {
            if (request is null)
            ...
            var productsWithThumbnails = await (from p in _repositoryService.Table<DhnProduct>()
                                                where p.CategoryId == 12 // <-- ĐÂY LÀ ĐIỂM KHÁC BIỆT
                                                select new
                                                {
                                                    Product = p,
